# atari_warpper from: https://github.com/openai/baselines/blob/master/baselines/common/atari_wrappers.py
...
class GymUpdate(gym.Wrapper):
    """support both old/new gym"""
    def __init__(self, env: gym.Env) -> None:
        super().__init__(env)
    ...
    def step(self, action: int):
        ...
        if len(result) == 5:
            ...
            # truncated=True will manage in Environment class
...
class WHC2CWH(gym.ObservationWrapper):
    def __init__(self, env: gym.Env):
        super().__init__(env)
        prev_shape = self.observation_space.shape
        self.observation_space = gym.spaces.Box(low=0, high=255, shape=(prev_shape[-1], *prev_shape[:-1]), dtype=np.float32)

    def observation(self, observation: np.ndarray):
        return np.swapaxes(observation, -1, 0)
...
class FrameStack(gym.Wrapper):
    """
    Stack k last frames.
    Use lazy array, which is much more memory efficient.
    """
    def __init__(self, env: gym.Env, k: int):
        ...
        self.observation_space = gym.spaces.Box(low=0, high=255, shape=(prev_shape[0]*k, *prev_shape[:-1]), dtype=np.float32)
